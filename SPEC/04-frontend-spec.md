...
**Deployment Note (Refactor v2):** The frontend is deployed as a separate Cloud Run service behind the load balancer. The load balancer routes `/api/*` to the backend and all other paths to the frontend. As a static export, the frontend still relies on SPA fallback (serving `index.html`) for deep linking to dynamic routes like `/runs/[id]`.
